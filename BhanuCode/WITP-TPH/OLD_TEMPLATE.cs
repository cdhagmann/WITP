...
namespace WITP_TPH
{
    ...
    //Iterations set up
        int NoTotalIterations = NOTOTALITERATIONSVAL;           //iterations for outer loop
        int NoOfIterations = NOOFITERATIONSVAL;               //iterations for inner loop
        int TotalGenerations = 1;               // 
        int NumSwapIterations = NUMSWAPITERATIONSVAL;              //determines the max number of iterations to swap 
        int stopIter = STOPITERVAL;                       //determines the number of iterations to stop if there is no improvement in the solutions
        int MaxTechIter = 3;                    //the maximum number of allowed iterations to select technology
        ...
        //List<float> FTPutawayCostArray = new List<float>() { 120, 160 };
        //List<float> PTPutawayCostArray = new List<float>() { 80, 120 };
        //List<float> FTPickCostArray = new List<float>() { 120, 160 };
        //List<float> PTPickCostArray = new List<float>() { 80, 120 };

        List<float> gammaArray = new List<float>() { 0.0F, 0.5F, 1.0F, 2.0F };
        List<float> phiArray = new List<float>() { 0.75F, 1.0F };

        //List<int> BeginTimeArray = new List<int>() { 0, 7, 14 };    //sorted data in ascending order
        //List<int> EndTimeArray = new List<int>() { 2, 12, 25 };    //sorted data in ascending order
        //List<int> dueTimeArray = new List<int>() { 4, 14, 27 };    //sorted data in ascending order
            ...
            for (int _pra = 0; _pra < PickRateArray.Count; _pra++) //Pick rate array bounding for loop
                for (int _gamma = 0; _gamma < gammaArray.Count; _gamma++) //Gamma bounding for loop
                    for (int _phi = 0; _phi < phiArray.Count; _phi++) //Phi bounding for loop
                    {
                        ...
                        //FT_PUTCOST = FTPutawayCostArray[_ftpra];
                        //FT_PICKCOST = FTPickCostArray[_ftpra];
                        //PT_PUTCOST = PTPutawayCostArray[_ftpra];
                        //PT_PICKCOST = PTPickCostArray[_ftpra];
                        gamma = gammaArray[_gamma];                         //Percentage of fulll time workers allowed to work as part time at the warehouse
                        phi = phiArray[_phi];

                        //_beginTime = BeginTimeArray[_time];                 //Begin time period for fashion event in the time-horizon
                        //_endTime = EndTimeArray[_time];     //*********End time-period for fashion event in the time-horizon ...very important..check for every data set else it will throw out of memory exception
                        //_dueTime = dueTimeArray[_time];                   //Due date for fashion products to reach store..in the time-horizon
                        //if ((_beginTime >= _endTime) || (_beginTime >= _dueTime) || (_endTime > _dueTime)) continue;

                        //ReadInputData();

                        //if ((_beginTime >= _numTimePeriods) || (_dueTime > _numTimePeriods) || (_endTime >= _numTimePeriods)) continue;
        ...
        public void ReadInputData()
        {
            ...
            while (!string.IsNullOrEmpty(parameter = sr.ReadLine()))
            {
                ...
                ////to read and store holding cost of stores
                //parameter = sr.ReadLine();
                //if (parameter.Equals("StoreHoldingCost:["))
                //{
                //    int i = 0;
                //    int _pStoreId = -1;
                //    int _cStoreId = 0;
                //    int StoreCount = -1;
                //    _storeHoldingCost = new List<List<float>>();
                //    while (!string.IsNullOrEmpty(parameter = sr.ReadLine()))
                //    {
                //        if (parameter.Equals("]"))
                //        {
                ...
                //                _pStoreId = _cStoreId;// = Int32.Parse(parts[0]);
                //                _storeHoldingCost.Add(l);
                //                StoreCount++;
                //            }
                //            _storeHoldingCost[StoreCount].Add(float.Parse(parts[2]));
                //            i++;
                //        }
                ...
                ////to read and store holding cost of warehouses
                //parameter = sr.ReadLine();
                //if (parameter.Equals("WarehouseHoldingCost:["))
                //{
                //    int i = 0;
                //    int _pWareHouseId = -1;
                //    int _cWareHouseId = 0;
                ...
                //    _warehouseHoldingCost = new List<List<float>>();
                //    while (!string.IsNullOrEmpty(parameter = sr.ReadLine()))
                //    {
                //        if (parameter.Equals("]"))
                //        {
                ...
                //                _warehouseHoldingCost.Add(l);
                //                WareHouseCount++;
                //            }
                //            _warehouseHoldingCost[WareHouseCount].Add(float.Parse(parts[2]));
                //            i++;
                //        }
                //    }
            ...
            for (int v = 0; v < _numBvendors; v++)
            {
                if (rand.Next(1, 100) < 75)
                {
                    if (_numBasicShipmentsVtoW[T][v] > 0)  //checking if there is a positive shipment in that time period
                    {
                        ...
                        {
                            // int period = _numPutaway.IndexOf(_numPutaway.Min());

                            //if (_numBasicShipmentsVtoW[minTime][v] == 0)
                            //{
                            if (minTime != T)
                            {
        ...
        public float ImproveBasicOutboundSol()
        {
            ...
            for (int s = 0; s < _numStores; s++)
            {
                //rand.Next(1, Math.Max(2, (int)_numStores / 10));
                if (rand.Next(1, 100) < rand.Next(50, 75))         //percentage of selecting a store is set based on the total number of stores**********
                {
                    if (_numShipmentsWtoS[T][s] > 0)  //checking if there is a positive shipment in that time period
                    {
                        if (rand.Next(1, 100) < 20) //20% chance of consolidation based on distance
                        {
        ...
        List<int> Top5PutawayTechList = new List<int>() { 0, 0, 0, 0, 0 };       //List to store best 5 technologies 
        List<int> Top5PickTechList = new List<int>() { 0, 0, 0, 0, 0 };
