...
model.lambda_put = Param(initialize=rd.lambda_put[i])
model.lambda_pick = Param(initialize=rd.lambda_pick[j])
model.MHE = Param(initialize=rd.MHE[i])
model.C_put = Param(initialize=rd.C_put[i])
model.C_pick = Param(initialize=rd.C_pick[j])
...
model.alpha_put = Var(bounds=(0.0, None),
                     within=NonNegativeIntegers)

model.alpha_pick = Var(bounds=(0.0, None),
                     within=NonNegativeIntegers)
...
def objective_rule(model):
    ...
    # Cost of
    expr += summation(model.Cth_put, model.theta_put) + \
    ...
    MHE_Cost = sum(model.MHE * (model.alpha_put + model.beta_put[t])
                   for t in model.TIMES)
...
def ConstraintFour_rule(model, t):
    ...
    Four_expr2 = model.lambda_put * (model.alpha_put + model.phi_put * model.beta_put[t])
    return (Four_expr1 - Four_expr2 <= 0)

model.ConstraintFour = Constraint(model.TIMES)
...
def ConstraintFive_rule(model, t):
    ...
    Five_expr2 = model.lambda_pick * (model.alpha_pick + model.phi_pick * model.beta_pick[t])
    return (Five_expr1 - Five_expr2 <= 0)

model.ConstraintFive = Constraint(model.TIMES)
